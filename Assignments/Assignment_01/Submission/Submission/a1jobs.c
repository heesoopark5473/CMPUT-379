...
int run_command(char* prgm, char* p1, char* p2, char* p3, char* p4) {
    ...
    if (child == 0) {
      ...
      close(fd[0]);
      write(fd[1], &exe_return, sizeof(exe_return));
      close(fd[1]);
      ...
      if (child == (pid_t)(-1)) {
        ...
        usleep(200000);
        close(fd[1]);
        read(fd[0], &exe_return, sizeof(exe_return));
        close(fd[0]);
        if(exe_return == 0){
          cmdlist[count].index = count;
          cmdlist[count].pid = child;
          cmdlist[count].ppid = (long) getpid();
          strcpy(cmdlist[count].prgm, argv[0]);
          count = iadd(count);
        }
        exe_return = 0;
      }
...
int suspend_command(char * jobNo){
  ...
  for(int k=0; k<count; k++)
  {
      if(cmdlist[k].index == jobNo_int){
        ...
        printf("PARENT: sending SIGSTOP to suspend the process : %d\n\n", cmdlist[k].pid);
...
int resume_command(char* jobNo){
  ...
  for(int k=0; k<count; k++)
  {
      if(cmdlist[k].index == jobNo_int){
        ...
        printf("PARENT: sending SIGCONT to suspend the process : %d\n\n", cmdlist[k].pid);
        //printf("[%d] pid : %ld, ppid : %d, prgm : %s\n", cmdlist[k].index, cmdlist[k].pid, cmdlist[k].ppid, cmdlist[k].prgm);
      }
  }
  return 0;
...
int terminate_command(char* jobNo){
  ...
  for(int k=0; k<count; k++)
  {
      if(cmdlist[k].index == jobNo_int){
        ...
        printf("PARENT: sending SIGKILL to suspend the process : %d\n\n", cmdlist[k].pid);
